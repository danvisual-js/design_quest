...
// Fetch prices for a batch of symbols — handles stocks, ETFs, and indices
async function fetchPrices(symbols){
  const eg=JSON.stringify(Object.fromEntries(symbols.slice(0,2).map(s=>[s,{p:298.14,pc:295.95}])));
  // Provide context so Claude knows how to search for non-stock symbols
  const symList=symbols.map(s=>{
    if(s==="DJI")return"DJI (Dow Jones Industrial Average index)";
    if(s==="VIX")return"VIX (CBOE Volatility Index)";
    if(s==="SPY")return"SPY (S&P 500 ETF)";
    if(s==="QQQ")return"QQQ (Nasdaq-100 ETF)";
    return s;
  }).join(", ");
  try{
    const res=await fetch("https://api.anthropic.com/v1/messages",{
      ...
      body:JSON.stringify({
        ...
        messages:[{role:"user",content:`Use web search to look up the current real-time prices for: ${symList}.

After searching, reply with ONLY this JSON — no explanation, no markdown fences, just the raw object:
${eg}

Use the ticker symbol as the key. Replace example values with the actual current price (p) and previous close (pc) you find for all ${symbols.length} symbols.`}]
      })
    });
    const d=await res.json();
    // Extract text from both text blocks and tool_result content (which can contain searched text)
...
export default function StockScreener(){
  ...
  // Keep a ref to the latest curTab stocks so the mount effect doesn't close over stale state
  ...
  useEffect(()=>{
    ...
    setTimeout(()=>runRefresh(curTabRef.current.stocks),600);
  },[]);// eslint-disable-line
...
Use the actual current price from your search results.`}]
      ...
      const data=await res.json();
      // Extract text from both text blocks and any tool_result content blocks
